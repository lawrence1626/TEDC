...
def CONCATE(df_key, DB_A, DB_Q, DB_name_A, DB_name_Q):
    ...
    tStart = time.time()
    print('Reading file: QNIA_key, Time: ', int(time.time() - tStart),'s'+'\n')
    KEY_DATA_t = readExcelFile(data_path+'QNIA_key.xlsx', header_ = 0, acceptNoFile=False, index_col_=0, sheet_name_='QNIA_key')
    print('Reading file: QNIA_database, Time: ', int(time.time() - tStart),'s'+'\n')
    DATA_BASE_t = readExcelFile(data_path+'QNIA_database.xlsx', header_ = 0, index_col_=0, acceptNoFile=False)
    
    print('Concating file: QNIA_key, Time: ', int(time.time() - tStart),'s'+'\n')
    KEY_DATA_t = pd.concat([KEY_DATA_t, df_key], ignore_index=True)
    
    print('Concating file: QNIA_database, Time: ', int(time.time() - tStart),'s'+'\n')
    for d in DB_name_A:
