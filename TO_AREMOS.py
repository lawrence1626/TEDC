...
tStart = time.time()
print('Reading file: '+NAME+'_key, Time: ', int(time.time() - tStart),'s'+'\n')
df_key = readExcelFile(data_path+NAME+'_key.xlsx', header_ = 0, acceptNoFile=False, index_col_=0, sheet_name_=NAME+'_key')
try:
    print('Reading file: '+NAME+'_database, Time: ', int(time.time() - tStart),'s'+'\n')
    DATA_BASE_t = readExcelFile(data_path+NAME+'_database.xlsx', header_ = 0, index_col_=0, acceptNoFile=False)
except:
...
while part_from_year >= from_year:
    ...
    AREMOS_DATA = []
    if part_from_year == '':
        print('From the earliest year to year',part_to_year, '\n')
    else:
    ...
    aremos_data.to_csv(out_path+NAME+"_data"+part_from_year[-2:]+".txt", header=False, index=False, sep='|', quoting=csv.QUOTE_NONE, quotechar='') #

    print('Time: ', int(time.time() - tStart),'s'+'\n')
    if part_to_year == to_year and make_doc == True:
        aremos = pd.DataFrame(AREMOS)
        aremos.to_csv(out_path+NAME+"_doc.txt", header=False, index=False, sep='|', quoting=csv.QUOTE_NONE, quotechar='') #
    if part_from_year == from_year:
        break
